...
export default function RopaPage() {
  ...
  useEffect(() => {
    const fetchRopa = async () => {
      try {
        ...
        if (!res.ok) throw new Error(result.error || "โหลดข้อมูล ROPA ไม่สำเร็จ");
  ...
  useEffect(() => { setPage(1); }, [search, selectedStatus, selectedRisks, selectedParties, retention]);
  ...
  const filteredData = data.filter((item) => {
    ...
    const hasRetentionFilter =
      retention.start.year || retention.start.month || retention.start.day ||
      retention.end.year || retention.end.month || retention.end.day;
    ...
    if (hasRetentionFilter) {
      ...
      if (minDays > 0 && maxDays > 0) matchRetention = itemDays >= minDays && itemDays <= maxDays;
  ...
  // ================= STYLE MAP =================
  const riskMap = {
    Critical: { color: "bg-[#F0AFBE] text-[#BD263F]", icon: <ChevronsUp size={14} /> },
    "At Risk": { color: "bg-[#F3E3AE] text-[#A37D00]", icon: <ChevronUp size={14} /> },
    Stable:   { color: "bg-[#D1E7F0] text-[#0078A3]", icon: <ChevronsDown size={14} /> },
    Safe:     { color: "bg-[#B5DDD8] text-[#228679]", icon: <ChevronDown size={14} /> },
  };
  ...
  const colConfig = { activity: 2, parties: 1.25, purpose: 1.5, legal: 1.5, retention: 1.5, risk: 1, status: 1, action: 0.1 };
  const col = Object.values(colConfig).map((v) => `${v}fr`).join(" ");

  const filterCount =
    selectedStatus.length + selectedRisks.length +
    (retention.start.year || retention.start.month || retention.start.day ||
     retention.end.year || retention.end.month || retention.end.day ? 1 : 0);
    ...
    <div className="flex h-screen bg-gray-100 font-prompt overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden text-[12px] ">
        ...
        <div className="px-10 pt-6 pb-4 shrink-0">
          ...
          <div className="border-b border-gray-200 mb-4" /><br />
          ...
          <div className="flex flex-col md:flex-row md:justify-between gap-4 items-start md:items-center">
              ...
              >
                <span className="text-sm flex items-center gap-2 text-[#1C1B1F]">
                  ...
                  <span className="truncate">
                    {startDate && endDate ? (
                      isSameDay ? (
                        <>{formatDate(startDate)}{isToday && <span className="text-[#03369D]">(today)</span>}</>
                      ) : (
                        <>{formatDate(startDate)}<span className="text-gray-400 mx-1">-</span>{formatDate(endDate)}</>
                      )
                    ) : (
                ...
                <div className="absolute mt-2 w-[264px] bg-white border rounded-xl shadow-lg p-4 z-20">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={() => { setStartDate(""); setEndDate(""); }}
                      className="text-xs text-[#03369D] hover:underline">Clear all</button>
                  </div>
                  ...
                  <div className="flex justify-end mt-4">
                    <button onClick={() => setOpen(false)}
                      className="text-sm bg-[#03369D] text-white px-4 py-1.5 rounded-md hover:bg-[#012a7c]">Apply</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-4 items-center text-[12px]">
              ...
              <div className="flex items-center gap-2 border rounded-lg px-3 w-[304px] h-[40px] bg-white">
                ...
                <input type="text" placeholder="ค้นหาชื่อกิจกรรม" value={search}
              ...
              <div className="relative">
                ...
                <FilterModal
                  ...
                  selected={selectedParties} setSelected={setSelectedParties}
                  selectedStatus={selectedStatus} setSelectedStatus={setSelectedStatus}
                  selectedRisks={selectedRisks} setSelectedRisks={setSelectedRisks}
        ...
        <div className="flex flex-1 overflow-hidden text-[12px] px-10 pb-6 gap-0">
          <div className="flex-1 flex flex-col overflow-hidden min-w-[300px]">
            <div className="bg-white rounded-xl shadow p-3 flex-1 overflow-y-auto">
              {loading ? (
                <div className="text-center text-gray-400 py-6">กำลังโหลดข้อมูล...</div>
              ) : apiError ? (
          ...
          {selectedItem && <div className="w-1 cursor-col-resize bg-gray-200" onMouseDown={initResize} />}
          {selectedItem && (
