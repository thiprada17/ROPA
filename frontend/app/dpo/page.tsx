...
import {
  ...
  ChevronsUp, ChevronUp, ChevronsDown, ClockFading, CheckCircle,
  AlertTriangle, EllipsisVertical, Shield,
} from "lucide-react";
...
export default function DpoPage() {
  ...
  useEffect(() => { setPage(1); }, [search, selectedStatus, selectedRisks, selectedParties]);
  ...
  useEffect(() => {
    const fetchDpo = async () => {
      try {
        ...
        let result: any;
        try { result = JSON.parse(text); } catch { throw new Error("Backend ไม่ได้ส่ง JSON"); }
  ...
  const filtered = data.filter(item => {
    ...
    const matchSearch = String(item.activity || "").toLowerCase().includes(kw);
    const matchStatus = selectedStatus.length === 0 || selectedStatus.includes(item.status);
    const matchRisk = selectedRisks.length === 0 || selectedRisks.includes(item.risk);
    const matchParties = selectedParties.length === 0 || (item.parties || []).some(p => selectedParties.includes(p));
  ...
  const filterCount = selectedStatus.length + selectedRisks.length + selectedParties.length;

  const riskMap: Record<string, { color: string; icon: React.ReactNode }> = {
    Critical: { color: "bg-[#F0AFBE] text-[#BD263F]", icon: <ChevronsUp size={13} /> },
    "At Risk": { color: "bg-[#F3E3AE] text-[#A37D00]", icon: <ChevronUp size={13} /> },
    Stable: { color: "bg-[#D1E7F0] text-[#0078A3]", icon: <ChevronsDown size={13} /> },
    Safe: { color: "bg-[#B5DDD8] text-[#228679]", icon: <ChevronDown size={13} /> },
  };

  const statusMap: Record<string, { color: string; icon: React.ReactNode }> = {
    Pending: { color: "border border-gray-300 text-[#03369D]", icon: <ClockFading size={13} /> },
    Complete: { color: "border border-gray-300 text-[#1C635A]", icon: <CheckCircle size={13} /> },
    Revision: { color: "border border-gray-300 text-[#AC273C]", icon: <AlertTriangle size={13} /> },
  };

  const badge = "flex items-center justify-center gap-1 px-3 py-[4px] rounded-full text-[11px] font-medium min-w-[90px]";
              ...
              {openDate && (
                <div className="absolute mt-2 w-[264px] bg-white border rounded-xl shadow-lg p-4 z-20">
                  ...
                  <div className="flex flex-col gap-3">
                    {[["วันเริ่มต้น", startDate, setStartDate], ["วันสิ้นสุด", endDate, setEndDate]].map(([label, val, setter]: any) => (
                      <div key={label}>
                        <label className="text-[11px] text-gray-500 block mb-1">{label}</label>
                        <input type="date" value={val} onChange={e => setter(e.target.value)} className="border rounded-md px-2 py-1.5 text-[12px] w-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
                      </div>
                    ))}
                  </div>
                  <div className="flex justify-end mt-4">
                    <button onClick={() => setOpenDate(false)} className="text-[12px] bg-[#03369D] text-white px-4 py-1.5 rounded-md hover:bg-[#012a7c]">Apply</button>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="flex flex-1 overflow-hidden px-10 pb-6 gap-0">
          ...
          <div className="flex-1 flex flex-col overflow-hidden min-w-[300px]">
            <div className="bg-white rounded-xl shadow p-3 flex-1 overflow-y-auto">
              {/* Header */}
              <div className="grid gap-2 mb-2 pl-4 pr-2 text-center text-[11px]" style={{ gridTemplateColumns: colConfig }}>
                {["กิจกรรมประมวลผล", "ฝ่ายที่เกี่ยวข้อง", "วัตถุประสงค์", "ฐานกฎหมาย", "ความเสี่ยง", "สถานะ", ""].map((h, i) => (
                  <div key={i} className={`px-3 py-3 rounded-lg ${h ? "bg-[#03369D] text-white" : ""}`}>{h}</div>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-1">
                {loading ? (
                  ...
                  <div className="text-center text-red-500 py-8">{apiError}</div>
                ) : paginated.length === 0 ? (
                  <div className="text-center text-gray-400 py-8">ไม่พบข้อมูล</div>
                ) : (
                  paginated.map(item => (
                    <div
                      key={item.id}
                    ...
                    >
                      {/* กิจกรรม */}
                      <div className="truncate text-gray-800 font-medium pr-2">{item.activity}</div>

                      {/* ฝ่าย */}
                      <div className="flex items-center gap-1 overflow-hidden min-w-0">
                        {(item.parties || []).slice(0, 2).map((p, i) => (
                          <span key={i} className="bg-[#DFE9FF] text-[#03369D] px-2 py-1 rounded-md text-[10px] whitespace-nowrap shrink-0" title={p}>{p}</span>
                        ))}
                        {(item.parties || []).length > 2 && (
                          <span className="text-gray-400 text-[10px]">+{(item.parties || []).length - 2}</span>
                        )}
                      </div>

                      {/* วัตถุประสงค์ */}
                      <div className="truncate text-gray-600 pr-2">{item.purpose}</div>

                      {/* ฐานกฎหมาย */}
                      <div className="flex items-center gap-1 overflow-hidden min-w-0">
                        {(item.legal?.basis || []).slice(0, 1).map((l, i) => (
                          <span key={i} className="bg-[#DFE9FF] text-[#03369D] px-2 py-1 rounded-md text-[10px] truncate" title={l}>{l}</span>
                        ))}
                        {(item.legal?.basis || []).length > 1 && (
                          <span className="text-gray-400 text-[10px]">+{(item.legal?.basis || []).length - 1}</span>
                        )}
                      </div>

                      {/* ความเสี่ยง */}
                      <div className="flex justify-center">
                        <span className={`${badge} ${riskMap[item.risk]?.color || "bg-gray-100 text-gray-600"}`}>
                      ...
                      {/* สถานะ */}
                      <div className="flex justify-center">
                        <span className={`${badge} ${statusMap[item.status]?.color || "border border-gray-300 text-gray-600"}`}>
                      ...
                      <div className="flex justify-end">
                        <button className="p-1 hover:bg-gray-200 rounded" onClick={e => { e.stopPropagation(); setSelectedItem(item); }}>
            ...
            <div className="mt-4 flex justify-between items-center text-[12px] shrink-0">
              ...
              <div className="flex items-center gap-2">
                <ChevronLeft size={18} className={`cursor-pointer ${page === 1 ? "opacity-30 pointer-events-none" : "hover:text-blue-600"}`} onClick={() => setPage(p => p - 1)} />
                <span>Page</span>
                <input type="number" value={page} min={1} max={totalPages} onChange={e => { const v = Number(e.target.value); if (v >= 1 && v <= totalPages) setPage(v); }} className="w-12 border rounded px-2 py-1 text-center" />
                <span>of {totalPages}</span>
                <ChevronRight size={18} className={`cursor-pointer ${page === totalPages ? "opacity-30 pointer-events-none" : "hover:text-blue-600"}`} onClick={() => setPage(p => p + 1)} />
              </div>
            </div>
          </div>
          ...
          {/* Resize handle */}
          {selectedItem && (
            <div className="w-1 cursor-col-resize bg-gray-200 hover:bg-blue-300 transition mx-1" onMouseDown={initResize} />
          )}
