...
import { Lock, Unlock, ShieldCheck, Eye, User, UserCog } from "lucide-react";
...
const col = "1fr 1.2fr 1.3fr 0.8fr 1fr 0.7fr 1.2fr 1fr 1fr 1fr";
    ...
    return (
        <div className="overflow-x-auto">
            <div className="min-w-[900px]">
                ...
                >
                    {headers.map((h, i) => (
                        <div key={i} className="px-2 py-[11px] rounded-[6px] bg-[#03369D] text-white">
                            {h}
                        </div>
                    ))}
                </div>
                ...
                <div className="flex flex-col font-prompt">
                    {data.length === 0 ? (
                        <div className="text-center text-gray-400 py-8 text-[12px]">ไม่พบข้อมูล</div>
                    ) : (
                        data.map(item => (
                            <div
                                key={item.id}
                                className="grid items-center pl-4 pr-1 py-3 border-b hover:bg-gray-50 transition cursor-pointer text-[12px]"
                                style={{ gridTemplateColumns: col }}
                            ...
                            >
                                ...
                                <div className="truncate text-gray-600 pr-2" title={isHashed(item.password) ? "Password hidden" : item.password}>
                                    {isHashed(item.password) ? "••••••••" : item.password}
                                </div>
                                <div className="text-gray-600">{item.phone}</div>
                                <div className="truncate text-gray-500 pr-1" title={item.team}>มม.เกาะรังสิต...</div>
                                ...
                                <div className="flex justify-center">
                                    <span className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium border ${roleMap[item.role].color}`}>
                                        {roleMap[item.role].icon} {item.role}
                                    </span>
                                </div>

                                {/* Lock status */}
                                <div className="flex justify-center">
                                    <span className={`flex items-center gap-1 text-[11px] font-medium ${lockMap[item.lockStatus].color}`}>
                                        {lockMap[item.lockStatus].icon} {item.lockStatus}
                                    </span>
                                </div>
