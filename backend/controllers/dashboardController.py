...
async def _get_all_activities():
    ...
    print("DASHBOARD activities:", res.data)
    print("DASHBOARD error:", getattr(res, "error", None))
...
async def _resolve_parties(activity_id: str) -> list[str]:
    ...
    dept_ids = [r.get("department_id") for r in dept_rows if r.get("department_id")]

    if not dept_ids:
        return []

    dept_res = authDB().table("departments") \
        .select("id, department_name") \
        .in_("id", dept_ids) \
        .execute()

    departments = safe_data(dept_res, [])

    return [
        strip_department_prefix(d.get("department_name"))
